...
def select(listings, cfg=None, already_sent=None, active_ids=None):
    """
    ...
    return cands[: int(_cfg(cfg, "bonanza_max_per_run"))]
...
# נקודת כניסה
# ------------------------------------------------------------------
def run(out_dir="out", cfg=None, download=True):
    ...
    picks = select(listings, cfg, already_sent=sent.keys(), active_ids=active_ids)
    if not picks:
...
if __name__ == "__main__":
    ...
    res = run(download=False)  # בסשן מקומי — בלי הורדת תמונות
    if not res:
        ...
        for r in res:
            ...
            print("\nתמונות:", *(r["image_urls"] or ["(אין)"]), sep="\n  ")
